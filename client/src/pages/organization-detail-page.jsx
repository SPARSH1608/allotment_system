...
const OrganizationDetailPage = () => {
    ...
    if (allotment.status === "Active") {
      return (
        <div className="text-xs text-gray-500">
          {diff > 0 ? `${diff} day${diff !== 1 ? "s" : ""} left` : diff === 0 ? "Due today" : "Overdue"}
        </div>
      );
    }
    if (allotment.status === "Overdue") {
      ...
      return (
        <div className="text-xs text-red-500">
          {overdueDays > 0 ? `${overdueDays} day${overdueDays !== 1 ? "s" : ""} overdue` : "Due today"}
        </div>
      );
    }
    return null;
  ...
  const handleGenerateInvoice = () => {
    const returnedAllotments = (allotments || []).filter((a) => a.status === "Returned");
    ...
    <div className="p-4 sm:p-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        <div className="bg-white p-3 sm:p-6 rounded-lg border border-gray-200">
          <div className="text-xs sm:text-sm font-medium text-gray-500">Contact Person</div>
          <div className="text-xs sm:text-lg font-semibold text-gray-900">{organization.contactPerson}</div>
        </div>
        <div className="bg-white p-3 sm:p-6 rounded-lg border border-gray-200">
          <div className="text-xs sm:text-sm font-medium text-gray-500">Email</div>
          <div className="text-xs sm:text-lg font-semibold text-gray-900">{organization.contactEmail}</div>
        </div>
        <div className="bg-white p-3 sm:p-6 rounded-lg border border-gray-200">
          <div className="text-xs sm:text-sm font-medium text-gray-500">Phone</div>
          <div className="text-xs sm:text-lg font-semibold text-gray-900">{organization.contactPhone}</div>
        </div>
        <div className="bg-white p-3 sm:p-6 rounded-lg border border-gray-200">
          <div className="text-xs sm:text-sm font-medium text-gray-500">Total Revenue</div>
          <div className="text-xs sm:text-lg font-semibold text-green-600">
      ...
      <div className="bg-white rounded-lg border border-gray-200">
        ...
        <div className="bg-white rounded-lg shadow p-4 sm:p-6">
          <h2 className="text-xl sm:text-2xl font-semibold mb-4">Allotments</h2>
                ...
                {filteredAllotments && filteredAllotments.length > 0 ? (
                  filteredAllotments.map((allotment) => (
                    <tr key={allotment._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-2 py-1 border">
                        {allotment.surrenderDate ? new Date(allotment.surrenderDate).toLocaleDateString() : "-"}
                      </td>
                      <td className="px-2 py-1 border">{getStatusBadge(allotment)}</td>
